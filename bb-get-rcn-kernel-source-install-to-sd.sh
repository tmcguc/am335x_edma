...
# 
# First of all, Robert C Nelson (rcn-ee.net) wrote this script, this is
...
# To use this script: 
# 
# <assumes you are using MicroSD card as an additional filesystem EXT4 works>
# Then: 
# mount /dev/mmcblk0p1 /media/microsd
# mv ./bb-get-rcn-kernel-source-install-to-sd.sh /media/microsd
...
install_kernel_source () {
	...
	mkdir -p ${SDDIR}/usr/src/
	rm -rf "${SDDIR}/usr/src/linux-${KVER}"

	mv "${DDIR}/linux-${MAIN_KVER}"	"${SDDIR}/usr/src/linux-${KVER}"
	mv "${SYMVERS}" "${SDDIR}/usr/src/linux-${KVER}/Module.symvers"
	mv "${KCONFIG}" "${SDDIR}/usr/src/linux-${KVER}/.config"

	mkdir -p "${SDDIR}/lib/modules/${KVER}"
	rm -f "${SDDIR}/lib/modules/${KVER}/build"
	ln -s "${SDDIR}/usr/src/linux-${KVER}" "/lib/modules/${KVER}/build"
}
